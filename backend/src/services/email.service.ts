...
function adminNotificationHtml(submission: Submission): string {
  ...
  const rowsHtml = rows
    .map(
      ([label, value]) =>
        `<tr><td style="padding:6px 12px;color:#617174;">${label}</td><td style="padding:6px 12px;">${escapeHtml(value)}</td></tr>`,
    )
    .join('');
  ...
  return `
    <div style="font-family:Arial,sans-serif;color:#102f45;max-width:560px;">
      ...
      <p style="color:#617174;font-size:12px;">
...
function founderConfirmationHtml(submission: Submission): string {
  return `
    <div style="font-family:Arial,sans-serif;color:#102f45;max-width:560px;">
      ...
      <p>
        Thank you for sharing "${escapeHtml(submission.title)}" with Maruthi Catalyst.
        Our team will review it confidentially and reach out when there's a thoughtful next step.
      </p>
...
export const emailService = {
  // Best-effort: a failure here must never take down a submission
  // that has already been safely stored in the database.
  async notifyAdminOfSubmission(
    ...
    try {
      ...
      if (error) {
        throw new Error(
          `Resend admin email failed: ${error.message}`,
        );
      }
  ...
  async sendFounderConfirmation(
    ...
    try {
      ...
      if (error) {
        throw new Error(
          `Resend founder email failed: ${error.message}`,
        );
      }

      logger.info('Founder confirmation email sent', {
        submissionId: submission.submissionId,
        emailId: data?.id,
      });
    } catch (err) {
